$(document).ready(function () {
    ...
    const selectionSummary = $('#bulkSelectionSummary');

    const updateBulkActionState = () => {
        const hasSelection = $('.rowCheckbox:checked').length > 0;
        const actionSelectEl = $('#bulkActionSelect')[0];
        const hasAction = actionSelectEl ? actionSelectEl.selectedIndex > 0 && !!$('#bulkActionSelect').val() : false;
        const enableAction = hasSelection && hasAction;
        $('#bulkActionBtn').attr('data-ready', enableAction);
        if (selectionSummary.length) {
            if (!hasSelection) {
                selectionSummary.text('No employee selected');
            } else {
                const count = $('.rowCheckbox:checked').length;
                selectionSummary.text(`${count} employee${count > 1 ? 's' : ''} selected`);
            }
        }
    };

    updateBulkActionState();
    $('#employeeList').on('change', '.rowCheckbox', updateBulkActionState);
    $('#bulkActionSelect').on('change', updateBulkActionState);

    $('#bulkActionBtn').on('click', async function(event) {
        event.preventDefault();
        event.stopPropagation();
        var employeeUserIdArray = [];
        var bulkActionSelect = $('#bulkActionSelect');
        var selectedAction = bulkActionSelect.val();
        var isPlaceholder = bulkActionSelect.prop('selectedIndex') === 0;
        if (!selectedAction || isPlaceholder) {
        ...
        if (!employeeUserIdArray.length) {
            showToast('No employees selected.', 'bg-red-500');
            return;
        }
        ...
        let bulkUpdateRes;
        try {
            bulkUpdateRes = await apiCall('POST', 'api/employee/bulk_update', jsonObject);
        } catch (error) {
            await loadEmployees(search, page, limit);
            updateBulkActionState();
            showToast('Bulk action failed. Please try again later.', 'bg-red-500');
            return;
        }
    ...
    window.employeeBulkValidationHandled = true;
        ...
        $.each(empList, async function name(key, employee) {
            ...
            newTr += `
                <tr>
                    ...
                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500 company">${ employee.affiliated_company && employee.affiliated_company.code ? employee.affiliated_company.code : 'N/A' }</td>
