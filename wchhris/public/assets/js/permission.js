document.addEventListener('DOMContentLoaded', function () {
    ...
    // Re-apply permissions whenever the sidebar menu DOM changes
    const sidebar = document.querySelector('.app-menu');
    if (sidebar && typeof MutationObserver !== 'undefined') {
        let reapplyTimer = null;
        const observer = new MutationObserver(() => {
            clearTimeout(reapplyTimer);
            reapplyTimer = setTimeout(applyPermissionsToDOM, 30);
        });
...
function applyPermissionsToDOM() {
    ...
    if (main_permissions) {
        ['project', 'humanres', 'administration', 'payroll', 'emp_leaves'].forEach(module => {
    ...
    if (sub_permissions) {
        const subModules = [
            ...
            'emp_leaves', 'holiday_config', 'leave_request', 'leave_calendar', 'overtime_request',
        ];
