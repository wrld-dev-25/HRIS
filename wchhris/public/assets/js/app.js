...
// JavaScript to hide the loader after the DOM is loaded
...
function updateHorizontalMenus() {
    ...
    if (navbarMenu) {
        ...
        if (typeof applyPermissionsToDOM === "function") {
            applyPermissionsToDOM(); // Re-apply role-based visibility after rebuilding the menu
        }
...
/**
 * Function to initialize and manage table pagination using List.js.
 * 
 * @param {string} tableName - The name (ID or class) of the table container to be paginated.
 * @param {Array<string>} columnNames - An array of strings representing the column names (class names) to be used by List.js.
 * @param {number} displayLimit - The number of records to display per page. Defaults to 10 if not provided.
 */
function paginateTable (tableName = "", columnNames = [], displayLimit = 10) {
    ...
    var options = {
        valueNames: columnNames,
        page: displayLimit ?? 10,
        pagination: true,
        plugins: [
            ListPagination({
                left: 2,
    ...
    var table = new List(tableName, options).on("updated", function (list) {
        ...
        const totalRecordsElement = table.listContainer.querySelector(".total-records");
        const showingElement = table.listContainer.querySelector(".showing");
    
        //if (totalEmploysElement) totalEmploysElement.innerHTML = employeeTable.items.length;
        if (totalRecordsElement) totalRecordsElement.innerHTML = table.items.length;
    
        if (showingElement) showingElement.innerHTML = table.visibleItems.length;
