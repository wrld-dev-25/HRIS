...
class EmployeeProfileController extends AbstractController
{
    ...
    public function updateOvertimeRequest(Request $request): Response
    {
        ...
        if (is_array($response) && isset($response['error']) && $response['error'] === true) {
            ...
            if (!$empCode) {
                $this->addFlash('status', $responseMessage);
        ...
        // Redirect to success route if creation is successful
        if (!$empCode) {
            $this->addFlash('status', 'Overtime request created successfully.');
            return $this->redirectToRoute('app_overtime_request');
        }
  ...
  #[Route('/profile-create-overtime-request', name: 'create_overtime_request')]
    public function createOvertimeRequest(Request $request): Response
    {
        $session = $request->getSession();
        $token   = $session->get('token');
        $user_id = $session->get('user_id'); // add user_id from session
        $emp_id  = $request->request->get('emp_record_id') ?? null;
        $otHours = $request->request->get('ot_hours');
        $otDate  = $request->request->get('ot_date') ?? null;
        $reason  = $request->request->get('reason') ?? "";
        ...
        $formData = [
            'user_id'      => $user_id,   // 🔴 sino ang nag-create
            'emp_id'       => $emp_id,
            'overtime_date'=> $otDate,
            'ot_hours'     => $otHours,
            'ot_reason'    => $reason,
            ...
            'deleted'      => 0,
            'created_at'   => $now,
            'created_by'   => $user_id,   // 🔴 created by current user
        ];

        $response = $this->apiService->apiRequest(
            'POST',
            'api/overtime_requests/create',
