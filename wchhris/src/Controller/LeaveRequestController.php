...
class LeaveRequestController extends AbstractController
{
    ...
    /**
     * Normalize the employee payload so the template can always iterate over
     * `emp_id`, `emp_fullname`, and `emp_code`.
     */
    private function mapEmployeesForDropdown(array $rawEmployees): array
    {
        $options = [];
        foreach ($rawEmployees as $employee) {
            if ($employee instanceof \Traversable) {
                $employee = iterator_to_array($employee);
            } elseif (!is_array($employee)) {
                $employee = (array) $employee;
            }

            $empId = $employee['id'] ?? $employee['emp_id'] ?? null;
            if ($empId === null) {
                continue;
            }
            ...
            $fullName = trim($lastName !== '' || $givenNames !== '' ? sprintf('%s, %s', $lastName, $givenNames) : (string) ($employee['emp_fullname'] ?? ''));
            if ($fullName === '') {
            ...
            $options[] = [
                ...
                'emp_code' => (string) ($employee['employee_code'] ?? $employee['emp_code'] ?? ''),
            ];
        }
    ...
    public function viewLeaveRequests(Request $request): Response
    {
        ...
        $employeeRecords = $this->decodeApiResponse($this->apiFunctions->getEmployees($request), 'employee list');
        $rawEmployees = $employeeRecords['employees'] ?? [];
        if ($rawEmployees instanceof \Traversable) {
            $rawEmployees = iterator_to_array($rawEmployees);
        } elseif (!is_array($rawEmployees)) {
            $rawEmployees = (array) $rawEmployees;
        }
        $employees = $this->mapEmployeesForDropdown($rawEmployees);
        if (empty($employees) && isset($employeeLeaves['emp_with_leave_list']) && is_array($employeeLeaves['emp_with_leave_list'])) {
    ...
    public function updateEmployeeLeaves(Request $request): Response
    {
        ...
        if (is_array($response)) {
            $errorMessage = $response['message'] ?? 'Unable to submit leave request.';
            return $this->redirectToRoute('app_leave_request', [
                'status' => $response['status'] ?? Response::HTTP_BAD_REQUEST,
