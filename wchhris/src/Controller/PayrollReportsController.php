...
class PayrollReportsController extends AbstractController
{
    ...
    private function getDatesAndCompany(Request $request): array
    {
        ...
        $companyName = $this->resolveCompanyName($request, $companyId);
        if ($companyId === null && $companyName === null) {
            $companyName = 'All Companies / Agencies';
        }
        return [$dateFrom, $dateTo, $companyId, $companyName];
    }

    private function resolveCompanyName(Request $request, ?string $companyId): ?string
    {
        if ($companyId === null) {
            return null;
        }
        ...
        try {
            $companies = $this->apiFunctions->getCompanyList($request)->toArray();
        } catch (\Throwable $e) {
            $this->logger->warning('Unable to resolve company name for payroll report export.', [
                'companyId' => $companyId,
                'error' => $e->getMessage(),
            ]);
            return null;
        ...
        foreach ($companies as $company) {
            $id = isset($company['id']) ? (string)$company['id'] : null;
    ...
    public function exportManpowerReport(Request $request): Response
    {
        ...
        [$dateFrom, $dateTo, $companyId, $companyName] = $this->getDatesAndCompany($request);
        ...
        return $this->exportXLSService->generateTimeSheetReport($dateFrom, $dateTo, $rows, $companyName);
    }
    ...
    public function exportPayrolls(Request $request): Response
    {
        ...
        return $this->exportXLSService->generatePayrollSheetReport($dateFrom, $dateTo, $data, $companyName);
    }
    ...
    public function exportPayrollsRegister(Request $request): Response
    {
        ...
        $data = $response->toArray();
        $data = $this->filterByCompany($data, $companyId);
        return $this->exportXLSService->generatePayrollRegisterReport($dateFrom, $dateTo, $data, $companyName);
    }
    ...
    public function exportTaxShield(Request $request): Response
    {
        ...
        $data = $response->toArray();
        $data = $this->filterByCompany($data, $companyId);
        return $this->exportXLSService->generateTaxShieldReport($dateFrom, $dateTo, $data, $companyName);
    }
    ...
    public function exportCashAdvance(Request $request): Response
    {
        ...
        $data = $response->toArray();
        $data = $this->filterByCompany($data, $companyId);
        return $this->exportXLSService->generateCashAdvanceReport($dateFrom, $dateTo, $data, $companyName);
    }
    ...
    public function exportSalaryAdjustment(Request $request): Response
    {
        ...
        $data = $response->toArray();
        $data = $this->filterByCompany($data, $companyId);
        return $this->exportXLSService->generateSalaryAdjustmentsReport($dateFrom, $dateTo, $data, $companyName);
    }
    ...
    public function exportGovDues(Request $request): Response
    {
        ...
        $data = $this->filterByCompany($data, $companyId);
        return $this->exportXLSService->generateTotalContributionReport($dateFrom, $dateTo, $data, $companyName);
    }
    ...
    public function exportCompanyGovDues(Request $request): Response
    {
        ...
        // redundant but safe
        $data = $this->filterByCompany($data, $companyId);
        return $this->exportXLSService->generateContributionReport($dateFrom, $dateTo, $data, $companyName);
    }
    ...
    public function exportPayrollSummary(Request $request): Response
    {
        ...
        $data = $response->toArray();
        $data = $this->filterByCompany($data, $companyId);
        return $this->exportXLSService->generatePayrollSummaryReport($dateFrom, $dateTo, $data, $companyName);
    }
}
