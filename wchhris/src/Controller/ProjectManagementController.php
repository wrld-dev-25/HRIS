...
class ProjectManagementController extends AbstractController
{
    ...
    private function decodeApiResponse(mixed $raw): array
    {
        if (is_array($raw)) {
            return $raw;
        }
    ...
    {
        $subdivisionData = $this->decodeApiResponse($this->apiFunctions->getSubdivision($request));
        $phaseData = $this->decodeApiResponse($this->apiFunctions->getPhase($request));

        if (($subdivisionData['status'] ?? 200) !== 200) {
            $this->logger->warning('Unable to load subdivisions list', ['payload' => $subdivisionData]);
        }
