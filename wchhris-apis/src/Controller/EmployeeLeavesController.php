...
class EmployeeLeavesController extends AbstractController
{
    ...
    public function view_employee_leaves_list(Request $request): JsonResponse
    {
        ...
        foreach($employeeList as $employee){
            $employeeLeaveData= [];
            $employeeLeaveList = $this->entityManager->getRepository(YearlyEmployeeLeave::class)
                ->findOneBy(['emp_record' => $employee, 'year' => $selectedYear]);
            if($employeeLeaveList){
                $selectedLeaves = $this->entityManager->getRepository(SelectedEmployeeLeaves::class)
                    ->findBy(['employee_leave' => $employeeLeaveList]);
    ...
    public function update_employee_leaves(Request $request): JsonResponse
    {
        ...
        $selectedLeaveIds = array_map('intval', $data['selected_leaves'] ?? []);

        $findExistingYearLeave = $this->entityManager->getRepository(YearlyEmployeeLeave::class)->findOneBy(['year' => $data['year'], 'emp_record' => $emp_record]);
        if(!$findExistingYearLeave){
        ...
        $existingSelectedLeaves = $this->entityManager->getRepository(SelectedEmployeeLeaves::class)->findBy(['employee_leave' => $emp_leave]);
        $existingByPolicy = [];
        foreach ($existingSelectedLeaves as $leave) {
            $existingByPolicy[$leave->getLeavePolicy()->getId()] = $leave;
        }

        foreach ($selectedLeaveIds as $leave_policy_id) {
            if (isset($existingByPolicy[$leave_policy_id])) {
                continue;
            }

            $selected_policy = $this->entityManager->find(LeavePolicy::class, $leave_policy_id);
            if(!$selected_policy){
                return new JsonResponse(['message' => 'Leave policy not found.'], JsonResponse::HTTP_BAD_REQUEST);
            }
        ...
        foreach ($existingByPolicy as $policyId => $leaveEntity) {
            if (!in_array($policyId, $selectedLeaveIds)) {
                $this->entityManager->remove($leaveEntity);
            }
        }
