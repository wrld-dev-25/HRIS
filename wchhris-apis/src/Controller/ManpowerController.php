...
class ManpowerController extends AbstractController
{
    ...
    public function bulkUpdateEmployee(Request $request): JsonResponse
    {
        ...
        $employeeUserIds = $this->normalizeEmployeeUserIds($data['employeeUserIds'] ?? []);
        ...
        try {
            switch ($action) {
                case 'is_project':
                    $this->runUserBulkUpdate($employeeUserIds, 'is_assignable_proj', true);
                    break;
    
                case 'remove_project':
                    $this->runUserBulkUpdate($employeeUserIds, 'is_assignable_proj', false);
                    break;
    
                case 'is_worker':
                    $this->runUserBulkUpdate($employeeUserIds, 'is_worker', true);
                    break;
    
                case 'remove_worker':
                    $this->runUserBulkUpdate($employeeUserIds, 'is_worker', false);
                    break;
                
                default:
                    return $this->json(['message' => 'Invalid bulk action supplied', 'status' => 'failed'], JsonResponse::HTTP_BAD_REQUEST);
            }
        } catch (\Throwable $e) {
            return $this->json(
                [
                    'message' => 'Bulk update failed. Please try again later.',
                    'status' => 'failed',
                    'error' => $e->getMessage(),
                ],
    ...
    public function bulkUpdateEmployeeV2(Request $request): JsonResponse
    {
        ...
        try {
            switch ($action) {
                ...
                case 'activate_acc':
                    $this->runUserBulkUpdate($employeeUserIds, 'is_active', true);
                    break;
    
                case 'deactivate_acc':
                    $this->runUserBulkUpdate($employeeUserIds, 'is_active', false);
                    break;
     ...
     * @param mixed $rawIds
     */
    private function normalizeEmployeeUserIds(mixed $rawIds): array
    {
        if (!is_array($rawIds)) {
            $rawIds = [$rawIds];
        }
    ...
    private function runUserBulkUpdate(array $employeeUserIds, string $field, bool $newStatus): void
    {
        $allowedFields = [
            'is_assignable_proj',
            'is_worker',
        ...
        if (!in_array($field, $allowedFields, true)) {
            throw new \InvalidArgumentException(sprintf('Unsupported bulk update field "%s".', $field));
        }

        $dql = sprintf(
            'UPDATE App\Entity\User e SET e.%s = :newStatus WHERE e.id IN (:employeeUserIds)',
            $field
        );
