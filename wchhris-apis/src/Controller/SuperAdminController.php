...
class SuperAdminController extends AbstractController
{
    ...
    public function createUserType(Request $request, EntityManagerInterface $entityManager): JsonResponse
    {
        try {
            ...
            $mainModules->setPermissions(
                ...
                $data['payroll'] ?? $defaultPermissions,
                $data['emp_leaves'] ?? $defaultPermissions,
            );
            ...
            return $this->json([
                ...
                'user_type' => [
                    ...
                    'main_module' => [
                        ...
                        'payroll' => $mainModules->getPayroll(),
                        'emp_leaves' => $mainModules->getEmpLeaves(),
                        'submodule' => $mainModules->getSubmodule() ? [
                            ...
                            'sss_config' => $mainModules->getSubmodule()->getSssConfig(),
                            'pagibig_config' => $mainModules->getSubmodule()->getPagibigConfig(),
                            'bir_config' => $mainModules->getSubmodule()->getBirConfig(),
                            'philhealth_config' => $mainModules->getSubmodule()->getPhilhealthConfig(),
                            'payroll' => $mainModules->getSubmodule()->getPayroll(),
                            'payroll_reports' => $mainModules->getSubmodule()->getPayrollReports(),
                            'leave_policy' => $mainModules->getSubmodule()->getLeavePolicy(),
                            'emp_leaves' => $mainModules->getSubmodule()->getEmpLeaves(),
                            'holiday_config' => $mainModules->getSubmodule()->getHolidayConfig(),
                            'leave_request' => $mainModules->getSubmodule()->getLeaveRequest(),
                            'leave_calendar' => $mainModules->getSubmodule()->getLeaveCalendar(),
                            'overtime_request' => $mainModules->getSubmodule()->getOvertimeRequest(),
                        ] : null,
                    ],
                ]
    ...
    public function updateMainModules(Request $request, EntityManagerInterface $entityManager, int $id): JsonResponse
    {
        try {
            ...
            // Ensure main-module leaves flag is explicitly set (handles all-false payloads)
            $mainModules->setEmpLeaves($data['emp_leaves']);
            $mainModules->setPayroll($data['payroll']);

                // Update SubModules if provided
                if (isset($data['submodule'])) {
                    $subModuleData = $data['submodule'];

                    // Normalize / fill missing submodules rather than failing hard
                    $requiredSubModules = ['daily_time_record', 'subdivision', 'division', 'department', 'phase', 'owner', 'models', 'model_types', 'emp_settings', 'shifts', 'projects','emp_project','emp_list','sss_config', 'pagibig_config', 'bir_config', 'philhealth_config', 'payroll', 'payroll_reports', 'leave_policy', 'emp_leaves', 'holiday_config', 'leave_request', 'leave_calendar', 'overtime_request'];
                    $defaultPermissions = [
                        'can_view' => false,
                        'can_add' => false,
                        'can_edit' => false,
                    ...
                    // Build normalized permissions (use incoming values if present, otherwise default to false)
                    $normalized = [];
                    foreach ($requiredSubModules as $field) {
                        $source = $subModuleData[$field] ?? $defaultPermissions;
                        $normalized[$field] = [
                            'can_view'   => isset($source['can_view'])   ? boolval($source['can_view'])   : false,
                            'can_add'    => isset($source['can_add'])    ? boolval($source['can_add'])    : false,
                            'can_edit'   => isset($source['can_edit'])   ? boolval($source['can_edit'])   : false,
                            'can_delete' => isset($source['can_delete']) ? boolval($source['can_delete']) : false,
                        ];
                    }

                    // Update SubModules entity with normalized data
                    $subModules->setPermissions(
                        $normalized['daily_time_record'],
                        ...
                        $normalized['owner'],
                        $normalized['models'],
                        $normalized['model_types'],
                        $normalized['emp_settings'],
                        $normalized['shifts'],
                        $normalized['projects'],
                        $normalized['emp_project'],
                        $normalized['emp_list'],

                        $normalized['sss_config'],
                        $normalized['pagibig_config'],
                        $normalized['bir_config'],
                        $normalized['philhealth_config'],
                        $normalized['payroll'],
                        $normalized['payroll_reports'],
                        $normalized['leave_policy'],
                        $normalized['emp_leaves'],
                        $normalized['holiday_config'],
                        $normalized['leave_request'],
                        $normalized['leave_calendar'],
                        $normalized['overtime_request'],
                        
                    );
                    // Explicitly set filing-related fields to enforce saved state
                    $subModules->setLeavePolicy($normalized['leave_policy']);
                    $subModules->setEmpLeaves($normalized['emp_leaves']);
                    $subModules->setHolidayConfig($normalized['holiday_config']);
                    $subModules->setLeaveRequest($normalized['leave_request']);
                    $subModules->setLeaveCalendar($normalized['leave_calendar']);
                    $subModules->setOvertimeRequest($normalized['overtime_request']);
                    $entityManager->persist($subModules);
                }
